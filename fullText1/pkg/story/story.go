...
func Execute(args []string) error {
	...
	abstractFilePath := cmd.String("abstract", "", "Path to the abstract file (text, json, or yaml) generated by the 'abstract' command.")
	...
	if *abstractFilePath == "" {
		return fmt.Errorf("--abstract is required for story generation")
	}
	...
	if strings.HasPrefix(strings.ToLower(abstractFileName), "abstract-") && (strings.HasSuffix(strings.ToLower(abstractFileName), ".txt") || strings.HasSuffix(strings.ToLower(abstractFileName), ".json") || strings.HasSuffix(strings.ToLower(abstractFileName), ".yaml") || strings.HasSuffix(strings.ToLower(abstractFileName), ".yml")) {
		// Example: abstract-2025-11-17-20-59-12.txt/json/yaml -> log-2025-11-17-20-59-12.log
		...
		logFileName = strings.Replace(logFileName, ".yaml", ".log", 1) // Added for YAML
		logFileName = strings.Replace(logFileName, ".yml", ".log", 1)  // Added for YML
	} else {
	...
	abstractContent, thoughtSignature, err := file.ReadAbstractFile(*abstractFilePath)
	if err != nil {
		return fmt.Errorf("failed to read and parse abstract file '%s': %w", *abstractFilePath, err)
	}
	_ = thoughtSignature // TODO: use the thought signature in first paragraph
	...
	chapterCountPlanResult := getChapterCountFromGeminiForStory(getChapterCountForStoryInput) // Updated call
	if chapterCountPlanResult.Err != nil {
		return fmt.Errorf("failed to get total chapter count from Gemini for story generation: %w", chapterCountPlanResult.Err)
	}
	totalChapters := chapterCountPlanResult.Count
	accumulatedInputTokens := chapterCountPlanResult.InputTokens // Initialize with this call's tokens
	accumulatedOutputTokens := chapterCountPlanResult.OutputTokens
	accumulatedCost := chapterCountPlanResult.Cost
	...
	if finalOutputPath == "" {
		...
		if strings.HasPrefix(strings.ToLower(baseName), "abstract-") {
			...
			// Ensure text extension, handle both json, yaml and yml abstract files
			finalOutputPath = strings.Replace(finalOutputPath, ".json", ".txt", 1)
			finalOutputPath = strings.Replace(finalOutputPath, ".yaml", ".txt", 1) // Added for YAML
			finalOutputPath = strings.Replace(finalOutputPath, ".yml", ".txt", 1)  // Added for YML
		} else {
			timestamp := time.Now().Format("2006-01-02-15-04-05")
	...
	if _, err := os.Stat(finalOutputPath); err == nil {
		...
		if readErr != nil {
			...
			writtenChapterCountResult := getWrittenChapterCountFromGemini(getWrittenChapterCountInput) // Updated call
			accumulatedInputTokens += writtenChapterCountResult.InputTokens
			accumulatedOutputTokens += writtenChapterCountResult.OutputTokens
			accumulatedCost += writtenChapterCountResult.Cost
			...
			if writtenChapterCountResult.Err != nil {
				...
				chaptersAlreadyWritten = writtenChapterCountResult.Count
	...
	var currentHistory *aiEndpoint.HistoryTurn = nil
...
`,
		...
		for attempt := 0; attempt <= maxChapterRetries; attempt++ {
			...
			apiInput := aiEndpoint.CallGeminiAPIInput{
				...
				Prompt:        prompt,
				ThinkingLevel: thinkingLevel,
				PreviousTurn:  currentHistory,
			}
			apiResponse := aiEndpoint.CallGeminiAPI(apiInput) // Updated call
		...
		currentHistory = &aiEndpoint.HistoryTurn{
			UserPrompt:       prompt,
